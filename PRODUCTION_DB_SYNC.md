...
| `tenant_subscriptions.status` sin `'free'` en el ENUM | `migrations/20260906b_billing_free_status.sql` (DESPUES del anterior). Sin esto, asignarle `status: 'free'` a una empresa (ej. AVP, uso interno) falla. Recordar despues de correrla: `POST /api/billing/subscriptions/4` con `{ plan_id: 1, status: 'free' }` para que AVP quede igual que en local. |
